...
function SearchBox() {
  ...
  const Handlelocation = (event) => {
    Handleresults();
    const getLocation = event.target.value;
    setLocation(getLocation);
  };

  const Handlebeach = (event) => {
    const getBeach = event.target.value;
    setBeach(getBeach);
  ...
  const getAllPinned = () => {
    const pinned = [];
    if (pinnedArray) {
      pinnedArray.map((beach) => beach.map((b) => beachArray.map((beaches) => (beaches.id === b
        ? pinned.push(
          <li key={uuidv4()}>
            <FavoriteCard
              id={beaches.id}
            ...
            ? (
              <div className="arrow">
                ...
                <button onClick={() => { ref.current?.scrollIntoView({ behavior: 'smooth' }); }}>
                  <img className="arrow-icon" src={require('../../../Assets/Images/arrow.png')} alt="" />
                </button>
              </div>
            )
            : null}
