...
export default function BasicModal(props) {
  ...
  const getBeach = () => {
    if (props.beachName) {
      axios.get(`${process.env.API_BASE_URL}api/v1/beach/allbeaches`)
        .then((response) => {
          response.data.beaches.map((data) => (data.name === props.beachName ? setId(data.id) : null));
        })
        .catch((error) => {
  ...
  const addPost = () => {
    ...
    axios.post(
      `${process.env.API_BASE_URL}api/v1/post/newpost`,
      {
        content,
        beachId: id,
        userId,
  ...
  const getAllPosts = () => {
    axios.get(`${process.env.API_BASE_URL}api/v1/post/allposts`)
      .then((response) => {
        const Posts = [];
        response.data.posts.map((data) => (data.beachId === id
          ? Posts.push(data)
          : null));
        setPostArray(Posts);
      })
    ...
    >
      ...
      <Box sx={style}>
        <div className="first-half">
          <img src={props.image} className="modal-img" />
