...
function BeachCard(props) {
  ...
  const pinBeach = (id) => {
    ...
    axios.post(
      `${process.env.API_BASE_URL}api/v1/user/pinned/`,
      {
        user_id: userId,
        beach_id: id,
      },
          ...
          <div className="beach-card bg-dark card text-white">
            {checkLogin()
              ...
              >
                ...
                {check ? <i className="fas fa-heart text-danger" /> : <i className="far fa-heart" />}
            ...
            <ul className="list-group list-group-flush">
              ...
              <li key={uuidv4()} className="list-group-item text-dark">
                <span className="card-list-title">Amenities :</span>
                {' '}
                <ul>{getAmenities()}</ul>
              </li>
            </ul>
